...
4. Create the dataset structure.
   ...
   ```bash
   sudo zfs create -o canmount=off -o mountpoint=legacy -p backuppool1/push-received/workstation1
   ```
   ...
   ```bash
   sudo zfs create -o canmount=off backuppool1/push-received/workstation1/raw
   ```
   ...
   ```bash
   sudo zfs create -o canmount=off -o encryption=on -o keyformat=passphrase backuppool1/push-received/workstation1/encrypted
   ```
...
3. Perform restorability test on regular basis.
   ...
   ```bash
   sudo zfs create -o canmount=off -o mountpoint=legacy -p backuppool1/restore-test
   ```

   Pick a snapshot to test:

   ```bash
   zfs list -t snapshot -r backuppool1 | grep workstation1
   ```

   Load the encryption key:

   ```bash
   sudo zfs load-key -L file://system.key backuppool1/push-received/workstation1/raw/USERDATA/root_3vwzok
   ```

   Clone the snapshot:

   ```bash
   sudo zfs clone backuppool1/push-received/workstation1/raw/USERDATA/root_3vwzok@autosnap_2026-04-13_08:45:05_frequently backuppool1/restore-test/workstation1
   ```

   Mount and verify:

   ```bash
   sudo mount -t zfs backuppool1/restore-test/workstation1 /mnt
   ```
   ...
   ```bash
   sudo umount /mnt
   sudo zfs destroy backuppool1/restore-test/workstation1
   ```

   Unload the encryption key:

   ```bash
   sudo zfs unload-key -L backuppool1/push-received/workstation1/raw/USERDATA/root_3vwzok
   ```
